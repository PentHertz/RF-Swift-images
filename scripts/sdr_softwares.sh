...
function sdrangel_soft_fromsource_install() {
	...
	goodecho "[+] Patching LimeSDR plugin to link libusb (static-LimeSuite NEEDED fix)"
	local limesdr_plugin="/opt/install/sdrangel/lib/sdrangel/plugins/libinputlimesdr.so"
	if [ -f "$limesdr_plugin" ]; then
		command -v patchelf >/dev/null 2>&1 || install_dependencies "patchelf"
		if ! objdump -p "$limesdr_plugin" | grep -q 'NEEDED.*libusb-1.0'; then
			patchelf --add-needed libusb-1.0.so.0 "$limesdr_plugin"
			goodecho "[+] libusb-1.0.so.0 added to $limesdr_plugin"
		else
			goodecho "[+] libusb already linked, skipping patch"
		fi
	else
		criticalecho-noexit "[-] LimeSDR plugin not found at $limesdr_plugin — skipping patch"
	fi
